...
namespace SpaceBattle.Lib
{
    public class EndpointTest
    {
        ...
        public void IncorrectWorkTest()
        {
            ...
            const int gameID = 111;
            var id = Guid.NewGuid().ToString();
            _idgame.Add(gameID, id);
            var mre = new ManualResetEvent(false);
            var th = IoC.Resolve<ServerThread>("CreateWithStartThread", id);
            ...
            var br = System.Net.HttpStatusCode.BadRequest;
            ...
            Assert.Equal(br, ans.Result.StatusCode);
            Endpoint.Stop();
            var hardstop = IoC.Resolve<ICommand>("HardStop", id, () => { mre.Set(); });
            var sender = IoC.Resolve<ISender>("SenderGetByID", id);
        ...
        public void CorrectWorkTest()
        {
            ...
            var mess = new Mess("StartServer", gameID, parameters);
            var ok = System.Net.HttpStatusCode.OK;
            var ns = IoC.Resolve<object>("Scopes.New", IoC.Resolve<object>("Scopes.Current"));
            Endpoint.Run(ns);
            var rec = JsonContent.Create(mess);
            var ans = server.PostAsync("/message", rec);
            Assert.Equal(ok, ans.Result.StatusCode);
            Endpoint.Stop();
            var hardstop = IoC.Resolve<ICommand>("HardStop", id, () => { mre.Set(); });
            var sender = IoC.Resolve<ISender>("SenderGetByID", id);
            var send = IoC.Resolve<ICommand>("SendCommand", sender, hardstop);
